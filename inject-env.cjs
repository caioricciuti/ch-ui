...
const envVars = {
  ...
  VITE_CLICKHOUSE_USE_ADVANCED: process.env.VITE_CLICKHOUSE_USE_ADVANCED === "true",
  VITE_CLICKHOUSE_CUSTOM_PATH: process.env.VITE_CLICKHOUSE_CUSTOM_PATH || "",
  VITE_CLICKHOUSE_REQUEST_TIMEOUT: parseInt(process.env.VITE_CLICKHOUSE_REQUEST_TIMEOUT || "30000", 10),
  VITE_BASE_PATH: process.env.VITE_BASE_PATH || "/",
};

console.log("Injecting environment variables:");
console.log("VITE_CLICKHOUSE_URL:", envVars.VITE_CLICKHOUSE_URL ? "SET" : "NOT SET");
console.log("VITE_CLICKHOUSE_USER:", envVars.VITE_CLICKHOUSE_USER ? "SET" : "NOT SET");
console.log("VITE_CLICKHOUSE_PASS:", envVars.VITE_CLICKHOUSE_PASS ? "SET (hidden)" : "NOT SET");
...
if (basePath && basePath !== "/" && !basePath.startsWith("http")) {
  ...
  // Replace asset paths
  indexHtmlContent = indexHtmlContent.replace(/href="\/([^"]+)"/g, `href="${baseWithoutTrailing}/$1"`);
  indexHtmlContent = indexHtmlContent.replace(/src="\/([^"]+)"/g, `src="${baseWithoutTrailing}/$1"`);
