...
<div class="flex flex-col flex-1 min-h-0">
  ...
  {#if filters.length > 0 || partialView}
    <div class="flex items-center gap-1.5 flex-wrap px-2 py-1 border-b border-edge-subtle bg-surface shrink-0">
      {#each filters as f (f.column)}
        <span class="inline-flex items-center gap-1 pl-1.5 pr-0.5 py-0.5 text-[11px] rounded-md bg-accent-soft text-accent">
          <Filter size={10} class="shrink-0" />
          <span class="font-mono max-w-72 truncate" title={filterChipLabel(f)}>{filterChipLabel(f)}</span>
          <button
            class="p-0.5 rounded hover:bg-accent/20"
      ...
      {#if filters.length > 1}
        <button
          class="px-1.5 py-0.5 text-[11px] text-fg-3 hover:text-fg rounded hover:bg-hover"
          onclick={() => onclearfilters?.()}
        >Clear all</button>
      {/if}
      {#if partialView}
        <span class="flex items-center gap-1 text-[11px] text-warning" title="The result hit the row limit and this query can't be re-run as a subquery, so sorting and filtering only apply to the loaded rows.">
  ...
  {#if loading}
    <div class="flex flex-col items-center justify-center flex-1 gap-2 text-fg-3">
      <Spinner size="sm" />
      <span class="text-sm">Executing query...</span>
      {#if streamRows > 0}
        <span class="text-xs text-fg-4 tabular-nums">{formatNumber(streamRows)} rows received</span>
      {/if}
    </div>
  {:else if error && parsedError}
    <div class="flex-1 p-4 overflow-auto">
      <div class="flex items-start gap-3 rounded-md bg-danger-soft p-3.5">
        <AlertTriangle size={16} class="text-danger shrink-0 mt-0.5" />
        <div class="min-w-0 flex-1">
          {#if parsedError.name !== null || parsedError.code !== null}
            <div class="flex items-center gap-2 mb-1.5 flex-wrap">
              <span class="inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-semibold font-mono bg-danger/15 text-danger">
                {parsedError.name ?? 'ERROR'}{parsedError.code !== null ? ` · ${parsedError.code}` : ''}
              </span>
              {#if ongotoerror && parsedError.position !== null}
                <button
                  class="inline-flex items-center gap-1 px-1.5 py-0.5 text-[11px] rounded border border-danger/40 text-danger hover:bg-danger/15 transition-colors"
                  onclick={ongotoerror}
                >
                  <Crosshair size={11} />
          ...
          <div class="text-[13px] text-fg whitespace-pre-wrap break-words">{parsedError.message}</div>
          ...
          {#if parsedError.message !== parsedError.raw}
            <details class="mt-2">
              <summary class="text-xs text-fg-3 cursor-pointer select-none hover:text-fg">Full error</summary>
              <pre class="mt-1 text-[11px] text-fg-2 whitespace-pre-wrap break-words font-mono">{parsedError.raw}</pre>
            </details>
          {/if}
        </div>
      </div>
  ...
  {:else if meta.length === 0}
    <div class="flex flex-col items-center justify-center flex-1 gap-2 text-fg-4">
      <SquareTerminal size={28} class="text-fg-4" />
      <p class="text-sm">Run a query to see results</p>
      <p class="text-xs text-fg-4">Cmd/Ctrl+Enter to execute</p>
    </div>
  {:else}
    <!-- Tab content -->
    {#if activeTab === 'data'}
