<script lang="ts">
  ...
  import { logsHistogramData, histogramSelection, histogramIndexAt, formatHistogramInterval, type HistogramRange } from './histogram'
  ...
  function draw() {
    ...
    const plot = logsHistogramData(buckets, bucketSeconds, range)
    ...
    const opts: uPlot.Options = {
      ...
      plugins: [tooltipPlugin({
        ...
        formatValue: (_v, i, idx) => String(plot.series[i - 1].counts[idx]),
      })],
      scales: { x: { time: true, range: () => plot.bounds }, y: { range: (_u, _min, max) => [0, Math.max(1, max * 1.05)] } },
      axes: [
      ...
      series: [
        ...
        ...plot.series.map((series) => ({
          label: series.label,
          stroke: resolve(series.color),
          fill: resolve(series.color),
          width: 0,
          paths: bars,
          points: { show: false },
        })),
    ...
    chart = new uPlot(opts, [plot.x, ...plot.series.map((series) => series.stacked)], container)
  }
