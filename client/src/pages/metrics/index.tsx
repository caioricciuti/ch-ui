...
function MetricsPage() {
  ...
  }, [user, metricsData, lastFetched, fetchMetrics, location.pathname]);

  if (!user?.activeOrganization) {
    return null; // Or a placeholder component prompting the user to select an organization
  }
  ...
  const groupedMetrics: Record<string, Metric[]> = {
    Overview:
      metricsData?.filter((m) =>
        [
          ...
          "Query Count Last 14 Days",
          "Memory Usage last 24h (avg / 10 minutes)",
          "CPU Usage last 24h (avg / 10 minutes)",
          "Merge and PartMutation last 24h (avg)",
        ].includes(m.title)
      ) || [],
    Tables:
      metricsData?.filter((m) =>
    ...
    Queries:
      metricsData?.filter((m) =>
        [
          ...
          "History Queries",
          "Failed Queries",
          "Latest Common Errors",
          "Most Expensive Queries",
          "Most Expensive Queries by Memory",
          "New Parts Created",
          "Explain",
          "Query Cache",
        ].includes(m.title)
    ...
    Merges:
      metricsData?.filter((m) =>
        ["Merges", "Merge Performance", "Mutations"].includes(m.title)
      ) || [],
    More:
      metricsData?.filter((m) =>
        [
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <ScrollArea className="h-[calc(100vh-200px)]">
        ...
        <Outlet /> {/* Renders subpages like /metrics/queries */}
      </ScrollArea>
    </div>
  );
}
